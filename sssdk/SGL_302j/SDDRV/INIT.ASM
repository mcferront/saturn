...
;===============================================;
;	      Çuector table ì]ëó		;
; ÇrÇbÇrÇhÉxÉNÉ^(Level1:$64)ÇÕèëä∑ã÷é~		;
;===============================================;
	if	sw_MODEL_M
...
vtbinit_0:	move.l	(a0)+,(a1)+		; set vector#02Å`#31
		dbra	d7,vtbinit_0
...
;Åyã@î\Åzèâä˙âªéû MIDI Prg#ÇÕ ëSÇƒÇÃ MIDI channel Ç…Ç®Ç¢Çƒ "0" Ç∆Ç∑ÇÈ	*
...
;Åyã@î\ÅzÇgÇnÇrÇs ÇhÅ^Çe óÃàÊÇ "ÇO" ÉNÉäÉA				*
;Åyì¸óÕÅz a6   : work ram top addr				94/07/26*
;************************************************************************
	if	sw_MODEL_M
		global	HOST_IF_init
...
;		move.w	#0,_tmp_MIDI_ch(a6)	; word	; $0000 Å` $001F
...
only_management_number7:
	...
	?area_map_loop:
		...
		beq.s	?300				; jump if not âπêFBANK-ID(=0)
...
; Ü• 			ÇrÇbÇrÇo âπ åπ èâ ä˙ âª				Ü•
; Ü•Åyì¸óÕÅz a5   : SCSP[FH1005] I/O addr				Ü•
; Ü•ÅyîjâÛÅz all							Ü•
...
IMXL_all_off:	lea	SCSP_ISEL+1(a5),a1
		moveq	#16-1,d7		; loop size
		moveq	#0,d0			; DSP ì¸óÕ Level off data
