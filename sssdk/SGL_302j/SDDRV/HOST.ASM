...
;************************************************************************
;Åy ã@ î\ Åz ÉzÉXÉgÇ©ÇÁÇÃÉRÉ}ÉìÉhèàóù					*
; ready d7.w : Command data ( $0001Å` )				 	*
;************************************************************************
...
;@		move.w	d0,tmp_HIF_wk(a6)	;+push	4,12
		;-------------------------------;
		; Sound Driver èàóùÉRÉ}ÉìÉhÇÃóöó
		;-------------------------------;
...
Rireki_1:	movem.l	(SP)+,d0		; reg. pop
		...
		andi.w	#$FF,d7			;
		move.b	d7,d7
		bpl.w	seq_Prg			; jump if d7:$01Å`$09
		cmpi.w	#$90+1,d7
		bcc.w	CM_ret			; return if $0084Å`$FFFF
		andi.w	#$0f,d7			;
		add.w	d7,d7			;
		add.w	d7,d7			;
		jsr	CM_DRV(pc,d7.w)		;
...
;		jmp	CM_ret(pc)
	;---------------------------------------;
		...
		jmp	CM_ret(pc)
;-----------------------------------------------;
;   HOSTÉRÉ}ÉìÉhèàóùÉpÉâÉÅÅ[É^ÇÃ SCSP MIDI OUT	;
; hold : a0 = P1 address			;
;-----------------------------------------------;
	global		Rireki_ex
Rireki_ex:
...
Rireki_out_lp:	move.b	d0,d1
		...
		moveq	#37-1,d7		; ready F0Å`F7 size
...
;@		jmp	seq_volume		; #0C çÌèú 94/10/31
		jmp	DSP_clear(pc)		; #0C
		jmp	all_off(pc)		; #0D
		jmp	SEQ_PAN(pc)		; #0E
...
;		;===============================;
;		; Command = $01   ÇaÇfÇläJén	;
;		;===============================;
;		global	seq_start
;seq_start:	move.b	(a0)+,d0		; P1 ready d0 : ( 0 Å` 7 )
...
;		move.b	(a0)+,d2		; P5 î≠âπÉÇÅ[Éh
;		movea.l	a2,a0			; ready a0
;		jsr	xseq_start(pc)		; ready a0,a6,d0,d1
;seq_start_1:	clc
;		rts
;		;===============================;
;		; Command = $02   ÇaÇfÇlí‚é~	;
;		; P1 = î≠âπä«óùî‘çÜ 0 Å` 7	;
;		; P2 = 80H : with release off	;
;		;===============================;
;		global	seq_stop
;seq_stop:	move.b	(a0)+,d0		; P1 ready d0 : ( 0 Å` 7 )
...
;seq_stop_exit:	rts
;		;===============================;
;		; Command = $03  ÇaÇfÇlÉ|Å[ÉY	;
;		;===============================;
;		global	seq_pause
;seq_pause:	move.b	(a0)+,d0		; P1 ready d0 : ( 0 Å` 7 )
;		jsr	xseq_pause(pc)		; ready a6,d0
;		clc
;		rts
;		;===============================;
;		; Command = $04 ÇaÇfÇlÉ|Å[ÉYâèú;
;		;===============================;
;		global	seq_cntnu
;seq_cntnu:	move.b	(a0)+,d0		; P1 ready d0 : ( 0 Å` 7 )
...
;		;===============================;
;		global	seq_fade
;seq_fade:	move.b	(a0)+,d0		; P1 ready d0 : ä«óùî‘çÜ
...
seq_allstop:	jsr	xseq_allstop(pc)
		...
		;===============================;
		global	seq_tempo
seq_tempo:	move.b	(a0)+,d0		; P1 ready d0 : ( 0 Å` 7 )
...
;@		addq.w	#4,d1
;@		andi.w	#$3FF,d1
;@		move.w	d1,_MIDI_RCV_WRPT(a6)	; write offset pointer çXêV
		lea	_MIDI_RCV_BF(a6),a6	;
		adda.w	d0,a6			;
		move.l	(a0)+,(a6)+		; P1 : Priority & CMD
;@		move.b	(a0)+,(a6)+		; P2 : î≠âπä«óùî‘çÜ & MIDI ch
...
;-----------------------------------------------------------------------;
;									;
;	ÇcÇrÇoÇÃèâä˙âªÇÕ [MPRO] <--- $000 (NOP) Ç∆Ç∑ÇÈ			;
;			 íxâÑóÃàÊ <--- $6000				;
;									;
;-----------------------------------------------------------------------;
...
SEQ_PAN:	lea	bs_PWKTP,a6		; 68K Prg work top address
		move.w	(a0),d0			; P1 : î≠âπä«óùî‘çÜ ( 0 Å` 7 )
		andi.w	#$700,d0		;
...
SEQ_PAN_OFF_lp:	and.b	d0,(a0)			; set layer PAN (bit7) on
		...
		rts
		;-------------------------------; 0(L) Å` 40H(C) Å` 7FH(R)
SEQ_PAN_ON:	lsr.b	#2,d1			;	    Å´
		subi.b	#$10,d1			;	    Å´
		bcc.s	SEQ_PAN_0		;	    Å´
		not.b	d1			;	    Å´
		addi.b	#$10,d1			; 1FH(L)Å`10H(C)00HÅ`0FH(R)
SEQ_PAN_0:	;-------------------------------; d1.b = DIPAN ( 0 Å` 1FH )
		move.b	d1,d3			;
...
SEQ_PAN_ON_lp1:	move.b	d1,(a1)			; set layer PAN off 
		...
		;-------------------------------;
		; î≠âπíÜÉXÉçÉbÉgÇÃìØàÍä«óùî‘çÜ	; d0 = 000,200,400,..,E00
		; ÇÃëSÇƒÇ…ëŒÇµÇƒ PAN ÇçXêV	;
		;-------------------------------;
...
SEQ_PAN_ON_lp2:	cmp.w	_sl_kanri(a0,d4.w),d0	; 
		bne.s	SEQ_PAN_ON_1		; jump if not equal î≠ä«î‘çÜ

; A.Miyazawa	{

		btst.b	#PCM_flg,PSPN(a0,d4.w)		; PCM stream ÉXÉçÉbÉgÇ…ÇÕå¯Ç©ÇπÇ»Ç¢ÅI
		bne.s	SEQ_PAN_ON_1
...
SEQ_PAN_ON_1:	addi.w	#slot_wk_unit,d4
		...
		rts
;************************************************************************
;		ÇgÇnÇrÇsÉRÉ}ÉìÉhÅiÅêÇWÇOÅjÇbÇcÅ|ÇcÇ` ÉåÉxÉã		*
...
drv_CDlvl_ST2:	move.b	d0,mixer_wk_SCSP+17(a6)	;
		...
		rts
;************************************************************************
;		ÇgÇnÇrÇsÉRÉ}ÉìÉhÅiÅêÇWÇPÅjÇbÇcÅ|ÇcÇ` ÇoÇ`Çm		*
...
drv_CDpan_ST2:	move.b	d0,mixer_wk_SCSP+17(a6)	;
		...
		rts
;************************************************************************
;	ÇgÇnÇrÇsÉRÉ}ÉìÉhÅiÅêÇWÇQÅj SCSP Master Volume [MVOL] êßå‰	*
; P1 : 0(off) Å` $0F(Max)				 		*
;************************************************************************
		global	drv_vol
drv_vol:	move.b	(a0)+,d0		; P1
		...
		rts
;************************************************************************
;	ÇgÇnÇrÇsÉRÉ}ÉìÉhÅiÅêÇWÇRÅj DSP Micro Program change êßå‰	*
; P1 : Effect#						 		*
;************************************************************************
		external	CTRL_5B
		global		drv_effect
...
;************************************************************************
;	ÇgÇnÇrÇsÉRÉ}ÉìÉhÅiÅêÇWÇSÅj not deffinition			*
;************************************************************************
		global	drv_84
drv_84:		rts
;************************************************************************
...
;        D3      = no care						*
;        D2Å`D0  = PCM Stream#						*
; P2   : D7Å`D5  = Direct level [DISDL]					*
;        D4Å`D0  = Direct Pan   [DIPAN]					*
; P3   : D7Å`D0  = PCM start address bit19Å`12				*
; P4   : D7Å`D0  = 	//	     bit11Å`4  ( bit3Å`0 = always "0" )	*
; P5,6 : D15Å`D0 = PCM data sample ( memory use ) size High byte per 1ch*
; P7,8 : D14Å`D11 = [OCT]						*
;	 D9 Å`D0  = [FNS]						*
; P9   : D6Å`D3  = [ISEL] Rch ( stereo/mono )				*
; 	 D2Å`D0  = [IMXL] Rch ( stereo/mono )				*
; P10  : D6Å`D3  = [ISEL] Lch ( stereo )				*
; 	 D2Å`D0  = [IMXL] Lch ( stereo )				*
; P11  : D7Å`D0  = total level						*
;************************************************************************
		global	drv_PCM_st
drv_PCM_st:
...
drv_85_6ST:
	...
	rts
;************************************************************************
;		 #8A ÇoÇbÇl ÉXÉgÉäÅ[ÉÄçƒê∂ parameter change		*
;  P1 = PCM stream play#						*
;  P2 = DISDL & DIPAN							*
;  P3 = OCT & FNS (H)							*
;  P4 = OCT & FNS (L)							*
;  P5 = Right ISEL & IMXL						*
;  P6 = Left  ISEL & IMXL						*
;  P7 = Total level		1995/03/20 êVãK				*
;************************************************************************
		global	drv_8A
drv_8A:
		lea	slot_work(a6),a2	;
...
drv_86_2:	rts
;************************************************************************
;	    ÇgÇnÇrÇsÉRÉ}ÉìÉhÅiÅêÇWÇWÅjÇlixer Parameter change		*
; P1 : EFREG#							94/07/27*
; P2 : [EFSDL],[EFPAN]							*
;************************************************************************
		global	drv_88
...
; Ü±Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Üµ
; Ü•	HOST Command = $10 : Sound initial			Ü•95/02/20
; ÜπÜ£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£ÜΩ
		global	drv_10
drv_10:
		sr_push				; 95/02/28
		int_di				; 11/16
...
drv_10_3:	lea	bs_PWKTP,a6		; 68K Prg work top address
		...
		jsr	COEF_all_clr(pc)	;
		jsr	TEMP_all_clr(pc)	;
		jsr	MIXER_wk_wr(pc)		; Mixer ïúãA

drv_10_4:	move.b	(a0)+,d0		; P5 : Parameter
		subq.b	#1,d0			; MIXER èâä˙âª ?
		bne.s	drv_10_5		;
...
drv_10_5:	sr_pop
		rts
;@; Ü±Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Üµ
;@; Ü•	HOST Command = $12 : ÇpÉTÉEÉìÉh				Ü•
;@; ÜπÜ£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£ÜΩ
;@		external	xseq_qsound
...
;@		jmp	xseq_qsound(pc)
;@; Ü±Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Üµ
;@; Ü•	HOST Command = $11 : ÇRÇc ÉTÉEÉìÉh			Ü•
;@; ÜπÜ£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£ÜΩ
...
;@		jmp	xseq_3dsound(pc)
; Ü±Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Üµ
; Ü•	HOST Command = $89 : Hard ware check			Ü•
; ÜπÜ£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£ÜΩ
		global	drv_89
...
; Ü±Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Üµ
; Ü•	HOST Command = $89 : (#00) D-RAM 4Mbit R/W check	Ü•
; Ü•	HOST Command = $89 : (#01) D-RAM 8Mbit R/W check	Ü•
; ÜπÜ£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£ÜΩ
...
D_RAM_error:	stc
		rts
; Ü±Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Üµ
; Ü•	HOST Command = $89 : (#02) SCSP MIDI comunication check	Ü•
; ÜπÜ£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£ÜΩ
		global	HD_CHK_02
...
HD_CHK_02_3:	stc
		rts
; Ü±Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Üµ
; Ü•	HOST Command = $89 : (#03) sound out L&R check		Ü•
; Ü•			   : (#04) sound out L only check	Ü•
; Ü•			   : (#05) sound out R only check	Ü•
; ÜπÜ£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£ÜΩ
...
;***********************************************;
; Åyì¸óÕÅzd2.b : SEQ Bank# ( 0 Å` F )		;
;	  d3.b : SEQ Song# ( 0 Å` FF )		;
; ÅyèoóÕÅza2 : Seq. top address			;
;	  CY : error				;
; ÅyîjâÛÅzd2/d3/d4/d5/d6			;
;***********************************************;
...
seq_start:
		...
		bsr	pause_off
		bsr	all_damper_off			; ver-2.01,òAë±ÉäÉNÉGÉXÉgÇµÇΩÇ∆Ç´ëOÇÃã»ÇÃÉ_ÉìÉpÅ[ÉtÉâÉOÇoff
