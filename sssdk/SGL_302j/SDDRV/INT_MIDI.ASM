...
end_of_exclusive:
	...
	?error_of_exclusive:
		bset.b	#ERRb26,Mem_err_bit+0		; ïsê≥ MIDI EXCL Receive
		bsr	NACK_OUT
		rts
...
;-----------------------------------------------;
			.public		EXCL_00
EXCL_00:	bsr.w	get_size		; return d7.w ( 00 Å` 7FH )
...
; <<<< EOX set >>>>
		...
		addq.w	#1,d4			;
		andi.w	#$3FF,d4		;
		move.w	d4,_MIDI_OUT_WRPT(a6)	; + 1 çXêV
		clc
		rts
...
;-----------------------------------------------;
	public	EXCL_01
EXCL_01:	bsr.w	get_size		; return d7.w ( 00 Å` 7FH )
		bsr.w	get_st_addr		; return a1(=d2)
		bcs.w	NACK_OUT

EXCL_01_lp:	move.b	(a0)+,d0		; ----xxxx : data D7Å`D4
		move.b	(a0)+,d1		; ----zzzz : data D3Å`D0
		lsl.b	#4,d0			; xxxx0000
		andi.b	#$0F,d1			; 0000zzzz
		or.b	d1,d0			; d0.b ( D7Å`D0 )
		move.b	d0,(a1)+
		dbra	d7,EXCL_01_lp
...
get_size:	clr.w	d7			; clear
		...
		lsl.b	#4,d7			; xxxx0000
		andi.b	#$0F,d1			; 0000zzzz
		or.b	d1,d7			; d7.w(size-1) = 00H Å` FFH
		rts
;-----------------------------------------------;
...
get_st_adr_lp:	lsl.l	#4,d2			;
		...
		cmpi.l	#$101000,d2
		bcc.w	st_addr_er		; jump if d2.l ÅÜ 101000H
...
;;		cmpi.l	#$100000,d2
;;		bcs.w	st_addr_er		; jp if 80000H ÅÖ d2 < 100000H
;;	endif
