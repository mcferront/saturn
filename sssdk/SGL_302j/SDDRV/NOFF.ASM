...
;ÜÆÜ¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü≤
;Ü§     MIDI note off							 Ü§
;Ü§ Key on íÜÇÃ ìØàÍMIDI# Ç∆ Note# ÇÃëSÇƒÇÃ slot Ç…ëŒÇµÇƒ Key off Çé¿çs Ü§
;Ü§Åy ì¸  óÕ Åz a6 = CPU work top addr.					 Ü§
;Ü§		d1.w = MIDI 1st. byte	0 Å` 1FH ( MIDI ch# )		 Ü§
;Ü§		d2.b = MIDI 2nd. byte	0 Å` 7FH ( note )		 Ü§
;Ü§		d3.b = MIDI 3rd. byte	0 Å` 7FH ( velocity )		 Ü§
;Ü§Åy èo  óÕ Åz Ç»Çµ							 Ü§
;Ü§ÅyÉåÉWÉXÉ^Åz destroy : d0,d1,d4,d5,d6				 Ü§
...
;@		rts
		bset.b	#ERRa_8,Mem_DRVERR_FLG+0	; key-off Ç∑Ç◊Ç´ key-oníÜslot
		rts				; Ç™ë∂ç›ÇµÇ»Ç¢
...
NOFF_M2:
		...
		jmp	Note_on(pc)
NOFF_M6:
;@@		btst.b	#k_DMPR,k_MIDI_flg(a6,d5.w)	; ì¸óÕ MIDI ch 
;@@		beq.s	NOFF_M5				;   Damper on/off ?
;@@		;-------------------------------;
;@@		; set flg_DMPR ÅFÉ_ÉìÉpÅ[Ç…ÇÊÇÈÇjey-offñ¢é¿çsÉtÉâÉOÉZÉbÉg
;@@
;@@		moveq	#32,d7			;
;@@		sub.w	off_slot_cnt(a6),d7	; = key-oníÜ slotêî
;@@		beq.s	ret_12			;
...
;@@		cmp.w	_tmp_kanri(a6),d6	; î≠âπä«óùî‘çÜ equal ?
;@@		bne.s	NOFF_M7			;
;@@
...
polyphonic_note_off:
		...
		sub.w	off_slot_cnt(a6),d7		; = key-on slotêî
		bne.s	OF_0				; jump if key-on slot exist
		bra	er_20

		;-------------------------------------------------------;
		; key-on íÜÇÃ ìØàÍ MIDI ch & Note# slot Ç÷ÇÃ key-off é¿çs;
		;Åyì¸óÕÅz d7.w : key-on slot êî
		;-------------------------------------------------------;
...
OF_0:
	...
	?slot_loop:
		...
		bclr.b	#flg_non,sl_flag2(a4,d4.w)	; set Note-offíÜ mode
		...
		bclr.b	#flg_KON,sl_flag2(a4,d4.w)	; set key-offíÜ mode
		addq.w	#1,off_slot_cnt(a6)		; = key off slotêî + 1
...
;@		move.b	Mem_ELMNT_no(a1),d6	; = ÉGÉåÉÅÉìÉgêî  ÅFÇw
		beq.s	DFLTR_off_1		;
...
DFL_off_EG_2:	jsr	get_EXPTBS(pc)		; return d0.w : -$3F01Å`+$3F01
...
;		Ü±Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Üµ
;		Ü•  ÇoÇdÇfçXêV						Ü•
; 		Ü•Åyì¸óÕ&ï€éùÅz	a4   : slot work top			Ü•
;		Ü•		d4.w : slot work offset			Ü•
;		Ü•		d0 low : History offset point		Ü•
; 		Ü•ÅyîjâÛÅz	a1   : 					Ü•
;		Ü•		d0 High	, d3.l , d5.l , d6.l		Ü•
;		ÜπÜ£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£ÜΩ
		global	PEG_off
PEG_off:	swap	d0
		...
		beq.s	PEG_off_3
		;-------------------------------;
		jsr	get_EXPTB2(pc)		; return d0.w:-$3F01Å`+$3F01
		move.w	d0,d6
		swap	d6
		clr.w	d6
		...
		swap	d5			;
		clr.w	d5			;
		move.l	d5,PEG_level(a4,d4.w)	; set ìûíBÉåÉxÉã
		addi.l	#$80000000,d5		;
...
PEG_off_2:	move.l	d6,d6			;
		bpl.s	PEG_off_exit		;
PEG_off_4:	neg.l	d6
PEG_off_exit:	move.l	d6,PEG_RATE(a4,d4.w)	; set åXÇ´Å^4msec
		swap	d0
		rts
...
er_20:		bset.b	#ERRa10,Mem_DRVERR_FLG+0	; key-off Ç∑Ç◊Ç´ key-oníÜslot
		rts				; Ç™ë∂ç›ÇµÇ»Ç¢
