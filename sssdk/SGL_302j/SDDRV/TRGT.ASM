...
; rewrite 95/05/02Å`  Ver1.30aà»å„  MIDI ch 0 Å` 0FH only
;
...
;=======================================================
		...
		dc.b	"Ver2.04 96/01/25"		; ããóøì˙Ç≈Ç∑Ç».
...
trgt_top_1:
		...
		lea	IO_SCSP,a5		; å≈íË
		jsr	FH1005_init(pc)		;
		jsr	wk_RAM_init(pc)		; ÉèÅ[ÉNÇqÇ`Çlèâä˙âª
	...
	if	sw_MODEL_M
		...
		movea.l	knr_BANK_adr(a6),_activ_MIX_BNK(a6)
			; = ä«óùî‘çÜÅîÇOÅAMIDI ch#0 ÇÃâπêFBANK#0
		jsr	MIXER_wr(pc)		;
		jsr	YM3802_init(pc)		;
	endif
...
main_2A:	lea	$10(a0),a0		; = 00,10,20,30,..,70H + 10H
		dbra	d7,main_2B		;
		andi.b	#$7F,bs_SIFWK+OF_HNDSHK_EN	; set Sound èàóùäÆóπ
main_2C:
		;-------------------------------;
		;  É^ÉCÉ}Å[Ç` (É|Å[ÉäÉìÉOäƒéã)	;
		;-------------------------------;
		global	Timebase
Timebase:	move.w	RG_SCIPD(a5),d0		; Å`12
		add.b	d0,d0			; Å`8 Timer-A over flow ?
		bpl.w	main_st			; jump if < 2msec
		;-------------------------------;
	if	sw_MODEL_M
		external	LED_CTRL
		jsr	LED_CTRL(pc)		; for LED(äJî≠É{Å[Éh)
	endif
		;-------------------------------;
		;     É^ÉCÉ}Å[Ç` ÉäÉZÉbÉg	;
		;-------------------------------;
...
main_4:
		;-------------------------------;
;@		move.b	RG_DEBUG+1(a5),d1	; ëOâÒê›íËEGíl
;@		not.b	d1			; d1.b = 0(off) Å` 1F(max)
;@;@		andi.b	#$1f,d1			;
;@		andi.w	#$10,d1			;
;@		move.b	BIG_timer(a6),d0	;
;@		andi.w	#$1F,d0			;
...
F0_out_pt:	bsr.w	F0_out			; MIDI error èÓïÒ EXCL 
...
MIDI_OUT_lp:	cmp.w	_MIDI_OUT_WRPT(a6),d4	; word
	...
	if	sw_MODEL_M
		...
		addq.w	#1,d4			;
		andi.w	#$3ff,d4		;
		move.w	d4,_MIDI_OUT_RDPT(a6)	; çXêV
		bra.w	MIDI_OUT_lp
ret_33:		rts
;-----------------------------------------------------------------------;
; 	Active Sensing [FEH] ÇÃ [MIDI_OUT_BF] Ç÷ÇÃ buffering		;
;-----------------------------------------------------------------------;
		global	FE_out
FE_out:		move.w	FE_count(a6),d0		;
		...
		addq.w	#1,d0			;
		andi.w	#$3FF,d0		;
		move.w	d0,_MIDI_OUT_WRPT(a6)	; + 1 çXêV
ret_12:		rts
;-----------------------------------------------------------------------;
; 	CPU èàóù over [F4H] ÇÃ [MIDI_OUT_BF] Ç÷ÇÃ buffering		;
;-----------------------------------------------------------------------;
		global	F4_out
F4_out:		move.w	FUKA_cnt(a6),d0
		...
		addq.w	#1,d1
		andi.w	#$3FF,d1		;
		move.w	d1,_MIDI_OUT_WRPT(a6)	; + 1 çXêV
F4_OUT_1:	move.w	#0,FUKA_cnt(a6)		; clear
		rts
;-----------------------------------------------------------------------;
;		Error èÓïÒÇÃ [_MIDI_OUT_BF] Ç÷ÇÃ buffering		;
;-----------------------------------------------------------------------;
		global	F0_out
...
F0_out_a:	lea	EXCL_TB(pc),a0		;
		...
		moveq	#13-1,d7		; ready F0Å`F7 size
;-----------------------------------------------;
...
EXCL_OUT_lp:	move.b	(a1)+,(a2,d0.w)		; MIDI bufferring
		...
		andi.w	#$3FF,d0		;
		dbra	d7,EXCL_OUT_lp		;
		move.w	d0,_MIDI_OUT_WRPT(a6)	; çXêV
ret_11:		rts
...
EXCL_TB:	dc.l	$F0437900	; Excl status,YAMAHA ID,DIV,Target Dev ID
		...
		dc.l	0,0,0,0,0,0,0,0
;-----------------------------------------------------------------------;
;		ÇsÇuóp ÉTÉEÉìÉhèoóÕÇÃÉÇÉmÉâÉãÅ^ÉXÉeÉåÉIëŒâû		;
; ready d0.b								;
;-----------------------------------------------------------------------;
		global	TV_mono_chg
...
to_st_lp:	move.w	_sl_kanri(a4,d4.w),d5	; = î≠âπä«óùî‘çÜÅ~200H
...
to_mono_lp2:	andi.b	#$E0,SCSP_DISDLPN(a5,d4.w)	; DIPAN ---> Center
		...
		rts
;-----------------------------------------------------------------------;
;			ÇcÇrÇoÉäÉìÉJ Çcirect down load			;
;-----------------------------------------------------------------------;
	if	sw_MODEL_M
		global	DSP_LINK
DSP_LINK:
		...
		add.l	d2,d0
		move.l	d0,DFL_INST_addr(a6)	; = èuéûílãüããÉGÉäÉA top addr.

		movea.l	d0,a0		; = èuéûílãüããÉGÉäÉA top addr.
		moveq	#32/2-1,d7	;
		moveq	#0,d0		;
...
DSP_CPU:	move.b	BIG_timer(a6),d0
		...
		bne	DSP_CPU_exit
		clr.w	d7			;
		move.b	DFL_ELMNT_NO(a6),d7	; D-Filter ElementÇÃêî 0Å`32
; A.Miyazawa	{
...
main_A:
		...
		beq	DSP_CPU_exit
		;-------------------------------;
		;     Effect change èàóùíÜ	;
		;-------------------------------;
		moveq	#16-1,d7		; loop size
		lea	SCSP_EFSDLPN(a5),a0	;
EFCT_CHG_lp:	move.b	(a0),d1
		andi.b	#$E0,d1
		beq.s	EFCT_CHG_0		; [EFSDL]ÇópÇ¢Çƒ Fade out
		subi.b	#$20,(a0)		;   ( CD-DAïîÇÕèúÇ≠ )
...
?skip2:
		...
		move.l	d0,(a2)+		;
		move.l	d0,(a2)+		;
		; ÜÆÜ¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü≤
		; Ü§	[RBL,COEF,MADRS,MPRO] write to SCSP	Ü§
		...
		lea	DL_CEF(a0),a2		; = dest. EXB COEFÅ` data top
		lea	DSP_COEF(a5),a3		;
		move.w	#$500/4-1,d7		; loop size
CTRL_5B_EX_1:	move.l	(a2)+,(a3)+		; wr [COEF],[MADR],[MPRO]
		dbra	d7,CTRL_5B_EX_1		;
		; ÜÆÜ¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü≤
		; Ü§åWêîÉeÅ[ÉuÉãì]ëó write to DSP R/W area	Ü§
		; Ü§ d5:RBP data				Ü§
		; Ü§ d4:COEF table size : ÇmÅ~A00H		Ü§
		; Ü§ a2:EXB COEF table top			Ü§
		; Ü∂Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü¢Ü∫
		moveq	#0,d4			;
		move.b	DL_NCT(a0),d4		; = COEF tableêî
		move.b	DL_RBL(a0),d2		; = RBLEN ( 0,1,2,3 )
		move.l	#$4000,d3		;
		andi.w	#3,d2			;
		beq.s	CTRL_5B_5		; jump if R/W area = $4000
		lsl.l	d2,d3
CTRL_5B_5:	; d3.l = $4000,$8000,$10000,$20000 ( íxâÑóÃàÊÉTÉCÉY )
		swap	d5			; = 007F0000
		lsr.l	#3,d5			; = 000FE000 ( RBPÅ~2000H )
		add.l	d3,d5			; = èuéûílãüããÉGÉäÉA top addr.
		move.l	d5,DFL_INST_addr(a6)	;
		movea.l	d5,a0			; = åWêîÉeÅ[ÉuÉã top addr.
		moveq	#0,d0			;
		moveq	#40H/4-1,d7		;
CTRL_5B_EX_6:	move.l	d0,(a0)+		; = èuéûílãüããÉGÉäÉA clear
		dbra	d7,CTRL_5B_EX_6		;
		lsr.l	#2,d4			;
		beq.s	CTRL_5B_EX_5		; jump if COEF table nothing
		subq.w	#1,d4			; loop size
CTRL_5B_EX_4:	move.l	(a2)+,(a0)+		; COEF table ì]ëó
...
DSP_CPU_exit:	rts
;-----------------------------------------------------------------------;
;			âπíˆ(PLFO/PEG/BEND)êßå‰				;
; ç≈èâÇÃÇSmsec : slot#00 Å` #07 Çèàóù					;
;   éüÇÃÇSmsec : slot#08 Å` #15 Çèàóù					;
;   éüÇÃÇSmsec : slot#16 Å` #23 Çèàóù					;
; Å@éüÇÃÇSmsec : slot#23 Å` #31 Çèàóù					;
; ÇPÉñÇÃÉXÉçÉbÉgÇÕÇPÇUmsecÇ≤Ç∆Ç…êßå‰Ç≥ÇÍÇÈ				;
;-----------------------------------------------------------------------;
		global	slot_ctrl
...
slot_ctrl_lp:	move.w	d4,d5			; = 0,$20,$40,..,$3E0
		...
		move.b	PSPN(a4,d5.w),d0	; PCM_flg : PCM çƒê∂ mode ?
		bmi.w	next_slot		; jump if Yes
	if	ENGN
		move.b	sl_flag1(a4,d5.w),d0	; Engine busy ?
		bmi.w	next_slot		; jump if Yes
	endif
;@@		move.b	EG_Value(a4,d5.w),d0	; EGíl = off ?
;@@		beq.w	next_slot		; jump if Yes
		...
		clr.w	d5			;
		move.b	sl_MIDI(a4,d4.w),d5	; = MIDI ch# (0Å`0FH)
		lsl.w	#5,d5			; *20H
		or.w	_sl_kanri(a4,d4.w),d5	; + î≠âπä«óùî‘çÜÅ~200H
...
; A.Miyazawa	{
	...
	?300:	addi.w	#$100,d5		; $706 Å` $9B0 cent = +$200
	?200:	addi.w	#$100,d5		; $3DE Å` $704 cent = +$100
	?100:					; $000 Å` $3DC cent = +$000
...
; Ü•	     <<<< íPà éûä‘ìñÇΩÇËÇÃÇoÇkÇeÇnÇ…ÇÊÇÈÉ¬âπíˆéZèo >>>>		Ü•
; Ü≈Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£ÜÕ
; Ü•Åõ 	       d0.l [----] : work					Ü•
; Ü•  	       d1.l [----]						Ü•
; Ü•Åõ 	       d2.l [----] : work					Ü•
; Ü•  	       d3.l [----]						Ü•
; Ü•ÅõÅyì¸óÕÅz d4.w [hold] : slot offset ( $00,$20 Å` $7C0 )		Ü•
; Ü•Åõ	       d5.l [dstr] : work					Ü•
; Ü•Åõ 	       d6.l [----] : work		 			Ü•
; Ü•  	       d7   [Hold] : loop count					Ü•
; Ü•	       a0   [----]						Ü•
; Ü•  	       a1   [----]						Ü•
; Ü•	       a2   [hold]						Ü•
; Ü•	       a3   [----]						Ü•
...
; Ü•	       a5   [----]		 				Ü•
; Ü•	       a6   [----]						Ü•
; Ü•ÅõÅyèoóÕÅz d5.w [dstr] : É¬âπíˆ XXYYH ( XXH = âπíˆ , YYH = ÉZÉìÉg )	Ü•
...
plfo_action:
	...
	?d2_0:
		...
		neg.l	d6
		add.l	d6,d0				; Cent    <-- Å{ FRQR_wk
	?d2_0A:	add.l	d6,d0				; Cent    <-- Å{ FRQR_wk
	?d2_0B:	add.l	d6,d0				; Cent    <-- Å{ FRQR_wk
	?d2_0C:	add.l	d6,d0				; Cent    <-- Å{ FRQR_wk
...
;===============================================;
;  ÇoÇdÇfêßå‰Åiî≠âπíÜ,Key offå„Ç‡ä‹ÇﬁÅj		;
;	Pitch EG âπíˆ addition			;
;Åyì¸óÕÅz a2 : destination layer data top addr	;
;         a4 : slot work address                ;
;	  d4.w : slot work offset
;	  d5.w : Pitch
;ÅyèoóÕÅz d5.w : Pitch
;ÅyîjâÛÅz d0.l/d1.l/d3.w/a1			;
;===============================================;
...
PEG_6:		; <<<<<<<<<<< Release >>>>>>>>>>;
		...
		move.l	PEG_RATE(a4),d3			; set åXÇ´Å^16msec
		bne	ADD_RATE
...
PEG_5:		; <<<<<<<<<<< Sustain èIóπ Key-off ë“Çø >>>>>>>>>>;
PEG_7:		; <<<<<<<<<<< Release èIóπ >>>>>>>>>>;
...
PEG_123:	addq.b	#1,PEG_SEG(a4)			; set next Segment
		clr.w	d0
		move.b	(a1)+,d0			; = [DR]
		beq	RATE_00				; jump if [DR]ÅÅ 0

		cmpi.b	#$80,d0
		beq	RATE_80				; jump if [DR]=80H

	; [AR/DR]ÅÇ0 & ÅÇ80H åXÇ´óLå¯

			.global		RATE_ON
RATE_ON:
		bsr	get_EXPTB2			; return d0.w:-$3F01Å`+$3F01

		swap	d0
		clr.w	d0
		asr.l	#2,d0				; for 16msec
		move.l	d0,PEG_RATE(a4)			; set åXÇ´Å^16msec
		move.l	PEG_level(a4),d0
		move.l	d0,PEG_cent(a4)
		...
		move.w	d0,PEG_level(a4)		; set ìûíB Level high word
		moveq	#0,d0
		move.w	d0,PEG_level+2(a4)		; set ìûíB Level low word
...
; Ü±Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Üµ
; Ü• ÇrÇbÇrÇoÉ^ÉCÉ}Å[ÇaÅiÇrÇdÇpÉfÉRÅ[ÉhÅjäÑçûÇ›	Ü•
; ÜπÜ£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£Ü£ÜΩ
		global	level_2
level_2:
...
;xxxx		beq.s	level_2_exit		;
		...
		addi.w	#SEQ_LED_speed,SEQ_LED_cnt(a6)	; <<< ÇrÇdÇpâìÄäÑçû >>>
...
;--------------------------------------------------------------------------
	if	sw_MODEL_M
MAC_MIXER:
		move.b	BIG_timer(a6),d0	; êîÇPÇOmsecÇÃä‘äuÇÇ®Ç©Ç»Ç¢Ç∆
		andi.b	#$0F,d0			; MONO ÇÃÇ∆Ç´ÉmÉCÉYÇ™èoÇÈÅB
		bne	MAC_MIXER_pass		; 
...
OF_HNDSHK_flg_chk:
		...
		move.l	d1,(a0)+		; = nop,nop
		;-------------------------------------------------------;
		;  68k èàóù MIDI ÇÃ [MIDI_OUT_BF] Ç÷ÇÃ buffering 	;
		;-------------------------------------------------------;
init_01:	add.b	d0,d0			; bit 6 = H ?
		...
		move.l	d1,(a0)			; = nop,nop
		;-------------------------------------------------------;
		; 68k èàóù HOST Command ÇÃ [MIDI_OUT_BF] Ç÷ÇÃ buffering;
		;-------------------------------------------------------;
init_02:	add.b	d0,d0			; bit 5 = H ?
		...
		move.l	d1,(a0)			; = nop,nop
		;-------------------------------------------------------;
		;	Error èÓïÒÇÃ [MIDI_OUT_BF] Ç÷ÇÃ buffering	;
		;-------------------------------------------------------;
...
init_06:	add.b	d0,d0			; bit 1 = H ?
		...
		;-------------------------------------------------------;
		;	MIDI OUT Enable ?				;
		;  68k Ç…ÇÊÇÈ [MIDI_OUT_BF] ÇÃ äOïîÇ÷ÇÃ MIDI out	;
		;-------------------------------------------------------;
...
ret_13:		rts
;-----------------------------------------------------------------------;
;   Carriage Return code check ( SDDRVS.TSK ÇÃ Mediaä‘Ç≈ÇÃï°êªcheck )	;
;-----------------------------------------------------------------------;
	global	CR_check
CR_check:	lea	CR_TB(pc),a0
		...
		cmpi.b	#$0A,(a0)+
		beq.w	ret_34
CR_chk_er:	bset.b	#ERRb20,Mem_err_bit+1	; SDDRVS.TSK ÇÃ Mediaä‘Ç≈ÇÃï°êªÉ~ÉX
ret_34:
		; <<<<< äOïî SCSP-MIDI ì¸óÕÇÃî≠âπä«óùî‘çÜÇÃê›íË >>>>>
...
;-----------------------------------------------;<<< a3 >>>
ENGN_wk_rpm	equ	0	; byte 0 Å` $22
ENGN_wk_accl	equ	1	; byte 0 Å` $0F
;		equ	2	; byte
;		equ	3	; byte
...
;-----------------------------------------------;<<< a0 >>>
ENGN_slot_TB:	dc.w	$300	; bit15=H : î≠âπ(égóp)íÜ
		dc.w	$320
		dc.w	$340
		dc.w	$360
		dc.w	$380
...
ENGN_CTRL:	lea	ENGN_wk_cmn(a6),a3	;
		...
		move.w	ENGN_P1_rpm(a1),d0	; = 0000HÅ`2260H
		beq.w	ENGN_all_off_s		; jump if RPM = 0000H
		move.b	ENGN_P1_rpm(a1),d0	; rpm = 00H Å` 22H
		cmp.b	ENGN_wk_rpm(a3),d0	; 
		bne.w	ENGN_CTRL_3		; jump if rpm ïœâª
		move.l	ENGN_P1_accl(a1),d1	;
		cmp.b	ENGN_wk_accl(a3),d1	; 
		beq.w	ret_21			; jump if rpm & accel ïœâªÇ»Çµ
		andi.b	#$0F,d1
ENGN_CTRL_3:	cmpi.b	#$23,d0			;
		bcs.w	ENGN_CTRL_2		; jump if 00H Å` 22H
		move.b	#$22,d0			
ENGN_CTRL_2:	move.b	d0,ENGN_wk_rpm(a3)	; çXêV
		move.b	d1,ENGN_wk_accl(a3)	; çXêV
		moveq	#16-1,d7		; loop size
...
ENGN_exe:
		...
		andi.w	#$F,d0				;
		move.b	ENGN_TB_accl(a2,d0.w),d2	; d2.w = âπó shift

		clr.w	d0				;
		move.b	ENGN_wk_rpm(a3),d0		; = $00 Å` $22
		moveq	#0,d1				;
		...
		bmi.w	ENGN_running		; jump if î≠âπíÜ
		clr.w	d0			;
		moveq	#8-1,d6			; loop size
ENGN_slt_srch:	move.w	(a0,d0.w),d4		; = $300,$320,...,$3E0
		bpl.w	ENGN_slt_find		; jump if off íÜ
		addq	#2,d0
...
ENGN_slt_find:	bset.b	#7,(a0,d0.w)		;
		...
		move.l	(a4)+,SCSP_PCM_LSA(a5,d4.w)	; [LSA]Å`
		move.l	(a4)+,SCSP_D2R1R(a5,d4.w)	; [D2R]Å`
		addq.l	#6,a4
		move.b	#$FF,SCSP_TLVL(a5,d4.w)
		move.l	(a4)+,SCSP_RELFO(a5,d4.w)
		move.w	(a4)+,SCSP_DISDLPN(a5,d4.w)

		bsr.w	ENGN_PITCH_WR		; <<<< âπíˆèëçû >>>>
		bsr.w	ENGN_TL_WR		; <<<< âπó èëçû >>>>
	global	ENGN_on
ENGN_on:	bset.b	#7,ENGN_TB_flg(a2)	; set î≠âπíÜ
		move.b	(a5,d4.w),d0		;
		ori.b	#%00011000,d0		; set   KYONB ( key-on èÄîı )
		move.b	d0,(a5,d4.w)		; write KYONEX & KYONB
		rts
		;-------------------------------;
		; ready d1.w : TL($FF:MinÅ`0:Max);
		; 	d2.w : âπó shift(80H,81H,82H,Å`,7EH,7FH);
		; 	d3.w : Pitch(0Å`3FFFH)	;
		;-------------------------------;
	global	ENGN_running
...
ENGN_TL_WR:	moveq	#0,d0
		...
		bcc.w	TL_max
		move.b	d1,d0
TL_max:		move.b	d0,SCSP_TLVL(a5,d4.w)	; <<<< âπó èëçû >>>>
		rts
		;-------------------------------;
		;-------------------------------;
ENGN_PITCH_WR:	move.w	ENGN_TB_d4(a2),d4	;
		andi.w	#$3FFF,d3		; 0000HÅ`2000H(44.1kHz)Å`3FFFH
		...
		add.w	d3,d3			;
		or.w	d0,d3			;
		move.w	d3,SCSP_OCTFNS(a5,d4.w)	; <<<< âπíˆèëçû >>>>
		rts
		;-------------------------------;
		;     <<<< Pitch = 00 >>>>	;
		;-------------------------------;
...
Pitch_00:	bclr.b	#7,ENGN_TB_flg(a2)	; bit7 = PCM on
		beq.w	Pitch_00_ret		; jump if off íÜ
		; <<<< key off >>>>
	global	ENGN_off
...
Pitch_00_ret:	rts
;===============================================;
;    âÒì]êî = 0 : all Activ Engine slots off	;
;===============================================;
...
ENGN_all_off_3:	move.b	d1,ENGN_TB_flg(a2)	;
		...
		moveq	#8-1,d7			; loop size
ENGN_all_off_2:	bclr.b	#7,(a0)			; î≠âπíÜ ?
...
ENGN_init:
		...
		bcs.w	ret_22			; jump if âπêFBANK-ID nothing
...
CTRL_12:	move.l	ENGN_wk_cmn+ENGN_wk_TONE(a6),d0	;
		...
		lea	$7C0,a1			;
		move.w	d0,ENGN_P1_rpm(a1)	; = 0000HÅ`2260H
ret_23:		rts
		;-------------------------------;
CTRL_13:	move.l	ENGN_wk_cmn+ENGN_wk_TONE(a6),d0	;
		...
		lea	$7C0,a1			;
		move.l	d3,ENGN_P1_accl(a1)	; = 0000HÅ`000FH
ret_24:		rts
RPM_CHG_TB:
