...
;		}
	...
	external	BANK_init
	external	EFSDL_all_off		; MIXER[EFREG0Å`F] clear

;************************************************************************
;		  ÇgÇnÇrÇsÉRÉ}ÉìÉhÅiÅêÇWÇVÅjÇlixer# change		*
; P1 : 0 Å` F ( âπêFBANK# )					94/07/27*
; P2 : 0 Å` 7F ( Mixer# )						*
;************************************************************************
	global	drv_87
drv_87:		move.b	(a0)+,d0		; P1 : âπêF BANK#
		move.b	(a0)+,d3		; P2 : Mixer#
		andi.w	#$7F,d3			;
		move.w	d3,_activ_MIX_NO(a6)	; ready Mixer#

		jsr	get_BANK_addr(pc)	; ready d0
		bcs	return_2		; jump if âπêFBANK-ID nothing
...
;Åyã@î\ÅzéwíËâπêFÇaÇ`ÇmÇj"SCSPBIN"ÇÃêÊì™ÉAÉhÉåÉXÇ Ç`rea Çlap ÉJÉåÉìÉg	*
;	 Ç©ÇÁåüçıÇµÇƒ[k_BANK_adr]Ç…äiî[ÅB				*
;	 Ç‹ÇΩ[k_PRG_no],[k_MIDI_PAN],[k_PBend_BF]Ç 0 clear		*
;Åyì¸óÕÅzd0.w : âπêFÇaÇ`ÇmÇjî‘çÜ ( $00Å`$0F )				*
;ÅyîjâÛÅza0/a1/d2/d3/d7						94/07/26*
;ÅyèoóÕÅzd2.l : âπêFÇaÇ`ÇmÇj top address				*
;************************************************************************
		external	er_1B
...
BNK_chg:	move.b	(a0),d2			; data exist ?
		bmi.s	er_IF_bit2		; jump if âπêFBANK-ID nothing
		move.b	d2,d3			; = data ID & ID#
		andi.b	#$70,d2			; = data ID only
		bne.s	BNK_chg_4		; jump if not âπêFBANK-ID(=0)
		andi.b	#$0F,d3			; = ID# only
		cmp.b	d0,d3			; BANK# equal ?
		beq.s	BNK_chg_3		; jump if Yes
BNK_chg_4:	lea	8(a0),a0		;
		dbra	d7,BNK_chg		;
		bra.s	er_IF_bit2		; jump if âπêFBANK-ID nothing

BNK_chg_3:	move.w	(a0)+,d2		;
		andi.w	#$0F,d2			; = addr D19Å`D16
		swap	d2			;
		move.w	(a0)+,d2		; = addr D15Å`D0
		move.b	(a0),d3			; ÉfÅ[É^ down load çœ ?
		bpl.s	er_IF_bit2		; jump if âπêFBANK not ready
;@		move.l	d2,_activ_MIX_BNK(a6)	;
...
return_2:	rts
;************************************************************************
;Åyã@î\Åz ÇlÇhÇcÇhÉRÉìÉgÉçÅ[ÉãÉ`ÉFÉìÉW[$Bn,$10,$xx]êßå‰ÅB		*
;	  éwíËÇlÇhÇwÇdÇqÉfÅ[É^[EFSDL]&[EFPAN]èëçûÇ›ÅB			*
;Åyì¸óÕÅzd1.w : MIDI ch# ( 0 Å` 1F )					*
;        d3.b : Parameter 00Å`7FH = MIXER Number		94/07/26*
;************************************************************************
		global	CTRL_10
CTRL_10:	; ïsì¡íËÅiîƒópëÄçÏéqÇPÅj
		move.w	_tmp_kanri(a6),d2	; = î≠âπä«óùî‘çÜÅ~200H
		lsl.w	#5,d1			; activ Mixer MIDI ch# 0Å`F
...
;************************************************************************
;			ÇlÇhÇwÇdÇq çXêV					*
;Åyã@î\ÅzÇlÇhÇwÇdÇqÉfÅ[É^Ç"mixer_wk"Ç…ì]ëóå„ÅAÇrÇbÇrÇoÇ…èëçûÅB		*
;Åyì¸óÕÅz[_activ_MIX_BNK] MIXÉfÅ[É^ÇÃÇ†ÇÈâπêFBANKêÊì™ÉAÉhÉåÉX		*
;	 [_activ_MIX_NO]  MIXER#					*
;************************************************************************
		global	MIXER_wr
MIXER_wr:
...
;@		bclr.b	#ERRb_0,Mem_err_bit+3
		;************************************************
		;    ÇlÇhÇwÇdÇq ÅiEffect Return & Pan ) çXêV	*
		;Åyã@î\Åz mixer_wk ÉfÅ[É^Ç "ÇdÇeÇqÇdÇf"Ç…èëçûÅB*
		;************************************************
		global	MIXER_wk_wr
MIXER_wk_wr:
...
er_IF_bit0:	bset.b	#ERRb_0,Mem_err_bit+3
		jmp	EFSDL_all_off(pc)	; MIXER[EFREG0Å`F] clear
;@		rts
...
;************************************************************************
		global	seq_map
seq_map:	move.b	(a0)+,d0		; P1 ready d0 : ( 0 Å` 7 )
		andi.w	#$FF,d0			;
...
;************************************************************************
;			    Ç`ÇqÇdÇ` ÇlÇ`Ço çXêV			*
;Åyã@î\ÅzéwíËÇ≥ÇÍÇΩÇlÇ`ÇoÇÉXÉyÉAóÃàÊÇ©ÇÁÉJÉåÉìÉgóÃàÊÇ…ì]ëóÅB		*
;Åyì¸óÕÅzd0.w : MAP#	( $00Å`$FF )					*
;ÅyèoóÕÅzCY = error / NC = complete				94/07/26*
;************************************************************************
		global	MAP_chg
MAP_chg:
		lea	bs_AMAPC,a0		; area map current work top
...
MAP_clear:	move.l	d4,(a0)+		; clear map current work 
		...
		move.w	#8*16-1,d7		; î≠âπä«óùî‘çÜêîÅ~MIDI-chêî
		moveq	#0,d4
...
sl_flg_clr:	move.b	d4,sl_flag1(a0)		; clear slot_work[sl_flag1]
		...
		dbra	d7,sl_flg_clr
		;-------------------------------;
		lea	bs_AREAM,a0		; Area Map ÉXÉyÉAóÃàÊ top
...
er_IF_bit1:	bset.b	#ERRb_1,Mem_err_bit+3	;
		rts
;************************************************************************
;Åyã@î\Åz ÇlÇhÇcÇhÉRÉìÉgÉçÅ[ÉãÉ`ÉFÉìÉW[$Bn,$20,$xx]êßå‰ÅB		*
;		<<  âπêF ÇaÇ`ÇmÇj É`ÉFÉìÉW >>				*
;		set dest. SCSPBIN top addr    ---> knr_BANK_adr		*
;		set MIDI Volume initial value ---> knr_MIDI_VOL		*
;		exe Prg change(Prg#=0)					*
;Åyì¸óÕÅzd1.w : MIDI ch# ( $00Å`$1F )					*
...
;		add.w	_tmp_kanri(a6),d0	; + î≠âπä«óùî‘çÜÅ~200H
...
;************************************************************************
;Åyã@î\Åz ÇlÇhÇcÇhÉvÉçÉOÉâÉÄÅiâπêFÅjÉ`ÉFÉìÉW[$Cn,$xx]êßå‰		*
;		set   MIDI Prg# 	     ---> knr_PROG_no		*
;		clear MIDI Pitch Bend buffer ---> knr_PBend_BF		*
;		set   Sequence PAN bit	     ---> knr_MIDI_PAN		*
;Åyì¸óÕÅzd1.w : MIDI ch# ( 0 Å` 1F )			       		*
;	 d2.b : MIDI Program change# ( 00 Å` 7FH )			*
;ÅydestroyÅza0/d0/d1						94/07/26*
;************************************************************************
...
;PRG_chg_PAN1:
;	if	sw_MODEL_M
;		cmpi.w	#$E00,_tmp_kanri(a6)	; = î≠âπä«óùî‘çÜÅÅÇVÅH
;		bne	PRG_chg_ret		;
;		lea	bs_MONTR,a0		;
;		add.w	d1,d1			;
;		add.w	d1,d1			;
