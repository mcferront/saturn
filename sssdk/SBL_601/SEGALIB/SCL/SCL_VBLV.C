 ...
 *  Updated by H.E on 1995-02-13 Ver.1.10
 *
...
static Sint32  ReqDisplayFlag    = 0;   /* Display Request flag              */
...
**■1995-07-11
**■1995-09-19	高橋智延
**	static となっているが、ヘッダで extern されているので、
**	static をはずす。
*/
volatile Uint16  SpFrameChgMode    = AUTO_FRAME_CHG; /* Frame Change Mode      */
volatile Sint32  SpFrameEraseMode  = 1;       /* Frame Erase  Mode             */
volatile Sint32  VBInterval        = 1;   /* V-Blank Interval for Frame Change */
static volatile Sint32  VBIntervalCounter = 0;   /* V-Blank Interval Counter          */
static volatile Sint32  ReqDisplayFlag    = 0;   /* Display Request flag              */
static volatile Sint32  ReqDisplayFlag2    = 0;   /* Display Request flag              */
static volatile Sint32  VBIntrDisableDepth = 0;  /* V-Blank Disable Depth             */
static volatile Sint32  frameChgFlag;
#else
volatile Uint16  SpFrameChgMode    = AUTO_FRAME_CHG;/* Frame Change Mode				*/
volatile Sint32  SpFrameEraseMode  = 1;				/* Frame Erase  Mode				*/
volatile Sint32  VBInterval        = 1;				/* V-Blank Interval for Frame Change*/
volatile Sint32  VBIntervalCounter = 0;				/* V-Blank Interval Counter			*/
volatile Sint32  ReqDisplayFlag    = 0;				/* Display Request flag				*/
static volatile Sint32  ReqDisplayFlag2    = 0;		/* Display Request flag				*/
static volatile Sint32  VBIntrDisableDepth = 0;		/* V-Blank Disable Depth			*/
static volatile Sint32  frameChgFlag;
#endif
...
{
    /** BEGIN ***************************************************************/
    if(SpFrameChgMode == MANUAL_FRAME_CHG) {
        VBIntervalCounter++;
...
void SCL_VblankEnd(void)
{
	...
	/** BEGIN ***************************************************************/
        if(SpFrameChgMode == NO_INTER_FRAME_CHG) {
            if(ReqDisplayFlag)   frameChgFlag = 2;
        } else
        if(SpFrameChgMode == NO_INTER_VBE_FRAME_CHG) {
            if(ReqDisplayFlag) {              /* set V-Blank Erase ON            */
        ...
        if(SpFrameChgMode == AUTO_FRAME_CHG) {
            frameChgFlag = 4;          /* add 95-02-13 H.E */
        }else {
            if(SpFrameEraseMode == ON)
                if((VBInterval - 1) <= VBIntervalCounter)
                    SPR_WRITE_REG(SPR_W_FBCR, SPR_FBCR_ERASE | SpFbcrMode);
                                                /* 次フレームイレーズオン*/
            if(VBInterval <= VBIntervalCounter) {
                if(ReqDisplayFlag) frameChgFlag = 1;
                VBIntervalCounter = 0;
            }
        }
	...
	if(SpDie) {
	    ...
	    if((frameChgFlag == 2)  &&
		    ...
		    if((frameChgFlag == 1) ||   /* add 95-02-13 H.E */
                       (frameChgFlag == 2))     /* add 95-02-13 H.E */
			SPR_WRITE_REG(SPR_W_FBCR, SPR_FBCR_MANUAL | SpFbcrMode);
	                                  /* スプライトのフレームバッファの切り替え */
		frameChgFlag = 0;
		ReqDisplayFlag2 = 0;
...
/*    frameChgFlag = 0;*/
	...
	SCL_ScrollShow();
        frameChgFlag = 4;          /* add 95-02-13 H.E */
        /* ReqDisplayFlag = 0; */  /* del 95-02-13 H.E */
    }else {
        VBIntervalCounter++;
        if(SpFrameEraseMode == ON) {
            if((VBInterval - 1) <= VBIntervalCounter) {
