...
;		ready d7.w : Elementêî
DSPEGLFO:
;@		rts
		subq.w	#1,d7			; loop size
		andi.w	#$1F,d7			;

		movea.l	DFL_INST_addr(a6),a0	; èuéûílèëçûÇ›ÉAÉhÉåÉX
...
DFL_EG_0:	; <<< EG start ready >>>
		...
		move.w	d0,(a0)			; <<< èuéûílèëçû >>>
		addi.w	#$8000,d0		;
		move.w	d0,DFL_add_wk(a2)	; = $0000Å`$FF00
...
DFL_EG_1:	; <<< EG start Hold >>>
		...
		;-------------------------------;
		global	set_next_seg
set_next_seg:	move.w	d4,DFL_add_wk(a2)	; = $0000Å`$FF00
		subi.w	#$8000,d4		; = -$8000Å`$7FFF
		move.w	d4,(a0)			; <<< èuéûílèëçû >>>
		move.b	6(a1,d3.w),d0		; = [AR,DR,SR,RR]
		jsr	get_EXPTBS(pc)		; return d0.w Å}$3FFF
		move.w	d0,DFL_add_bs(a2)	; set rate
		move.b	7(a1,d3.w),d0		; = [AL,DL,SL,RL]
		lsl.w	#8,d0			; = -$8000Å`+$7F00
		addi.w	#$8000,d0		;
		move.w	d0,DFL_level(a2)	; set level : $0000Å`$FF00
...
DFL_EG_3:	; <<< EG Decay >>>
DFL_EG_4:	; <<< EG Slope >>>
		move.w	DFL_level(a2),d4	; = level : $0000Å`$FF00
		move.w	DFL_add_wk(a2),d1	; = work  : $0000Å`$FFFF
		move.w	DFL_add_bs(a2),d0	; = rate  : Å}$3FFF
		bmi.s	DFL_EG_2A
		; <<< rate = + >>>
		add.w	d0,d1			; work + rate
		bcs.s	set_next_seg		;
		cmp.w	d1,d4			;
		bls.s	set_next_seg		; jump if work + rate ÅÜ level
		bra.s	DFL_new_wk
DFL_EG_2A:	; <<< rate = - >>>
		neg.w	d0			; 0Å`$3FFF
		...
		bcs.s	set_next_seg		;
		cmp.w	d1,d4			; level
		bcc.s	set_next_seg		; jump if work + rate ÅÖ level
		;-------------------------------;
		global	DFL_new_wk
DFL_new_wk:	move.w	d1,DFL_add_wk(a2)	; set new work
		subi.w	#$8000,d1		; = -$8000Å`+$7FFF
		move.w	d1,(a0)			; <<< èuéûílèëçû >>>
		bra.s	DSPEGLFO_next		;
...
ELMNT_LFO:	moveq	#0,d0			;
		...
		andi.w	#$ff00,d0		;
		lsr.w	#7,d0			;
		move.w	SINETB(pc,d0.w),d0	; ÅÅ èuéûíl -$8000Å`+$7FFF
		move.b	DFL_wk_AMP(a2),d1	; = 0Å`F
		andi.w	#$0F,d1			;
		beq.s	ELMNT_LFO_1
		cmpi.w	#$0F,d1
		beq.s	ELMNT_LFO_2
...
ELMNT_LFO_1:	move.w	d0,(a0)			; <<< èuéûílèëçû >>>
;@		bra.s	DSPEGLFO_next		;
		;===============================;
...
; Ü•		ÉTÉCÉìîgå`                                              Ü•
; Ü•                                                                    Ü•
; Ü•êUïù         ÅÅ Å|ÇWÇOÇOÇOÇgÅ`Å{ÇVÇeÇeÇeÇg                          Ü•
; Ü•ÉtÉHÅ[É}ÉbÉg ÅÅ ÇPÇUÉrÉbÉgÅAÇQÅfÇbÇoÇk                              Ü•
; Ü•ÇÜÅié¸ä˙Åj   ÅÅ ÇQÇTÇUÉ|ÉCÉìÉg                                      Ü•
...
;@;
;@;	ÇcÇrÇo ÇlÇnÇcópîgå` ( 7FFFH ÅÜ ê≥ÇÃóÃàÊÇÃÇ›ì‡Ç≈ÇÃêUïù ÅÜ 0010H
;@;     		bit format /  É…
;@;  		  16bit	     168Éñ 	  1.0 Hz = OCT = -8
;@;
;@SIN16_MOD:
