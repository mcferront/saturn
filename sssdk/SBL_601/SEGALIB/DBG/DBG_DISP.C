...
static void putCtrlChar(char c);
static void putwc(int x, int y, int color, char c);
#if	0
/*
**■1995-07-26	高橋智延
**	putc() は The Standard C で定義済み
*/
static void putc(int x, int y, int color, char c);
#else
static void	_dbg_putchar( int	x, int	y, int	color, char	c );
#endif
...
char *DBG_GetKeyStr(char *keyStr)
{
    ...
    padb = 0;
    i = 0;
    putCtrlChar(0x08); /* Corsol Display */
    for(;;) {
       	padc = *padData;
       	padh = padc & (padb ^ padc);
        padb = padc;
    ...
    befKeyY = y;
    winSwap(&keyWin);
    putCtrlChar(0x01); /* Corsol Delete */
    return keyStr;
}
...
/*****************************************************************************
 *
 * NAME:  putCtrlChar()  - Put Control Char 
 *
 * PARAMETERS :
 *
 *     (1) char  char       - <i> 文字データ
...
/*****************************************************************************
 *
 * NAME:  putwc()  - Put 1 Char to VRAM or Save Area for Pop Up Window
 *
 * PARAMETERS :
 *
...
/*****************************************************************************
 *
 * NAME:  _dbg_putchar()  - Put 1 Char to VRAM or Save Area （旧 putc() ）
 *
 * PARAMETERS :
 *
 *     (1) int   x, y           - <i> 文字書き込み座標
 ...
 *
 *	1995-07-26	高橋智延
 *		putc() は The Standard C で定義済みのため、関数名を putc() から
 *		_dbg_putchar() に変更
 *
 *****************************************************************************
 */
