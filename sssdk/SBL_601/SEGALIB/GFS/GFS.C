...
/* バージョン番号   */
const Sint8   gfs_version[GFS_VERSION_LEN] = GFS_VERSION_STR;
...
{
    if (gfs != NULL) {
        gfs_mngTermAccess(gfs, TRUE);         /* アクセス終了 */
        gfs_closeSub(gfs);
    }
...
GFS_LOCAL Sint32 gfs_waitRead(GfsHn gfs)
{
    ...
    while (1) {
        ...
        if ((GFS_TRN_STAT(trn) == GFTR_ST_END)&&
            ...
            /* 転送だけ先に終る場合がある                           */
            /* 再生範囲を越えた時はステータスをCOMPLETEDにするため  */
            /* BUSY中にbreakしないようにする                        */
            break;
        }
 ...
 *  アクセス動作が完了したか調べる
 *［備考］
 *  致命的エラーが発生した場合、無限ループになるのを避けるためTRUEを返す
 *****************************************************************************/
...
Sint32 GFS_NwStop(GfsHn gfs)
{
    ...
    gfs_mngTermAccess(gfs, wait_pause);      /* 完了処理 */
    ap = GFS_Tell(gfs);
...
Uint32  *GFS_StartTrans(GfsHn gfs, Sint32 *dadr)
{
    ...
    if (GFS_DTS_FTYPE(dtsrc) == CD_FILE) {
        GFCB_GetSctData(dpk, dtsrc);            /* セクタデータの取得   */
    }
...
{
    Sint32      ndir;
    Sint32      fid;        /* ルートディレクトリのファイル識別子   */
    GfsDirMng   *mng;

    if (GFS_IS_DDS() == TRUE) {   /* デモデモ実行中の場合     */
        if (dirtbl != NULL) {
            GFCD_SetCdbfs(TRUE);
        }
        ndir = GFDR_ChgRoot();
