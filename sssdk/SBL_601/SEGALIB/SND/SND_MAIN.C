...
#define COM_SET_3D_SOUND	0x11				/* 3D sound paramator set	*/
#define COM_SET_QSOUND		0x12				/* Qsound paramator set		*/
...
/*** サウンドドライバへのアクセスウェートマクロ(Dummy Read) */
/* 1996/07/10 Start (Dummy Read ADDRESS 6000000H) */
#define _WAIT_() do{\
int i,j;for(i=0;i<16;i++) j=*(volatile int *)0x06000000;\
}while(0)
/* 1996/07/10 End */
/* 1994/02/24 End */
...
******************************************************************************/
 ...
 SndRet SND_SetQSound(Uint8 QCh, Uint8 QSt)
 {
	...
	intrflag = 1;
	if(GetComBlockAdr() == OFF) HOST_SET_RETURN(SND_RET_NSET);
	SET_PRM(0, QCh);				/* paramater set          */
	SET_PRM(1, QSt);				/* paramater set          */
	SET_COMMAND(COM_SET_QSOUND);	/* command set            */
	HOST_SET_RETURN(SND_RET_SET);
}

/******************************************************************************
	NAME:    	SND_Set3D_Stat()		-	YAMAHA 3D SOUND
	PARAMETERS :
		(1) Uint8 Y3DDst		- <i>	YAMAHA 3D Distance
		(2) Uint8 Y3DAzm		- <i>	YAMAHA 3D Azimuth
		(3) Uint8 Y3DElv		- <i>	YAMAHA 3D Elevation
	DESCRIPTION:
...
******************************************************************************/
 ...
 SndRet SND_Set3D_Stat(Uint8 Y3DDst, Uint8 Y3DAzm, Uint8 Y3DElv)
 {
	...
	intrflag = 1;
	if(GetComBlockAdr() == OFF) HOST_SET_RETURN(SND_RET_NSET);
	SET_PRM(1, Y3DDst);				/* paramater set          */
	SET_PRM(2, Y3DAzm);				/* paramater set          */
	SET_PRM(3, Y3DElv);				/* paramater set          */
	SET_COMMAND(COM_SET_3D_SOUND);	/* command set            */
	HOST_SET_RETURN(SND_RET_SET);
}

/******************************************************************************
	NAME:    	SND_Set3D_Init()		-	YAMAHA 3D SOUND Initialize
	PARAMETERS :
		None
	DESCRIPTION:
