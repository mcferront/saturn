/*----------------------------------------------------------------------*/
/*	ÇrÇaÇkÇUÉTÉìÉvÉãÉQÅ[ÉÄÇæÇÊÅ[ÇÒ										*/
/*----------------------------------------------------------------------*/
...
unsigned short	flash_color[] = {	/* FlashÇÃêF	*/
			C_RGB(25,28,30),
			C_RGB(25,25,30),
			C_RGB(25,28,30),
			C_RGB(25,25,30),
...
unsigned short	org_color[] = {	/* å≥ÇÃêF	*/
			C_RGB( 6,16,25),
			C_RGB( 6, 6,25),
			C_RGB( 6,16,25),
			C_RGB( 6, 6,25),
...
void	player(	FIGHTER	*sts ){
	...
	if( sts->hitpoint != 0 ){					/* ê∂Ç´ÇƒÇ¢ÇÈ */
		if( sts->shotwait == 0 ){				/* çUåÇ */
...
void	init_objects( void ){
	...
	/* îÚÇ—éUÇÈÉ|ÉäÉSÉìÇÃèâä˙É|ÉWÉVÉáÉì	*/
	for( i = 0 ; i < 6 ; i++ ){
