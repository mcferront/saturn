...
#else
Sint32	BackUpWrite(Uint32 device,BupDir *dir,volatile Uint8 *data,Uint8 sw);
#endif
...
/* #if !(__GNUC__) */
			...
			sprintf((char *)buff,"Main Unit:Id=%d,Part=%d",
										CnTb[0].unit_id,CnTb[0].partition);
			FNT_Print256((volatile Uint8 *)SCL_VDP2_VRAM_A0,buff,20,40,2,0);
			sprintf((char *)buff,"Cartridge:Id=%d,Part=%d",
										CnTb[1].unit_id,CnTb[1].partition);
			FNT_Print256((volatile Uint8 *)SCL_VDP2_VRAM_A0,buff,20,60,2,0);
			sprintf((char *)buff,"Serial   :Id=%d,Part=%d",
										CnTb[2].unit_id,CnTb[2].partition);
...
/* #if !(__GNUC__) */
			...
			FNT_Print256((volatile Uint8 *)SCL_VDP2_VRAM_A0,buff,20, 80,2,0);
			sprintf((char *)buff,"Free Size   :%10ld",StTb.freesize);
			FNT_Print256((volatile Uint8 *)SCL_VDP2_VRAM_A0,buff,20,100,2,0);
			sprintf((char *)buff,"Free Block  :%10ld",StTb.freeblock);
			FNT_Print256((volatile Uint8 *)SCL_VDP2_VRAM_A0,buff,20,120,2,0);
			sprintf((char *)buff,"Data Num    :%10ld",StTb.datanum);
			FNT_Print256((volatile Uint8 *)SCL_VDP2_VRAM_A0,buff,20,140,2,0);
#endif
			PadInputWait();
			break;
		case	5:
...
/* #if !(__GNUC__) */
			...
			FNT_Print256((Uint8 *)SCL_VDP2_VRAM_A0,
										(Uint8 *)0x6080100,20,40,2,0);
			PadInputWait();
			((Uint8 *)0x6080100)[100] = work;
#else
			work = ((volatile Uint8 *)0x6080100)[100];
			((volatile Uint8 *)0x6080100)[100] = 0;
			FNT_Print256((volatile Uint8 *)SCL_VDP2_VRAM_A0,
								(Uint8 *)0x6080100,20,40,2,0);
			PadInputWait();
			((volatile Uint8 *)0x6080100)[100] = work;
#endif
			break;
		case	7:
			/* ファイルの削除 */
...
#else
			FNT_Print256((volatile Uint8 *)SCL_VDP2_VRAM_A0,buff,50,10,2,0);
#endif
			if(!num)	PadInputWait();
...
#else
			err_disp(BackUpWrite(device,&DirTb[0],
								(volatile Uint8 *)0x00200000,(Uint8 )ON),ON);
#endif
			dummy++;
			break;
		case	16:
...
/* #if !(__GNUC__) */
				...
				FNT_Print256((volatile Uint8 *)SCL_VDP2_VRAM_A0,
					(Uint8 *)"CPU Clock:26MHz", 20, 40,GREEN,0);
			else	FNT_Print256((volatile Uint8 *)SCL_VDP2_VRAM_A0,
					(Uint8 *)"CPU Clock:28MHz", 20, 40,BLACK,GREEN);
#endif
...
#else
	FNT_Print256((volatile Uint8 *)SCL_VDP2_VRAM_A0,buff,x,y,2,0);
#endif
}
